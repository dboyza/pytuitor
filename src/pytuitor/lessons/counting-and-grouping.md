...
The standard-library `collections` module provides useful dictionary variants.
`Counter` counts occurrences and treats an unseen key as zero.
...
Pass `list`, not `list()`, so a new empty list is created separately for each missing key.
`dict(...)` converts either variant to an ordinary dictionary for a simple return contract.
...
`pages[user]` is a list of their pages in encounter order, including duplicates.
