...
The `"x"` mode creates a new file exclusively and raises `FileExistsError` if it already exists.
...
It reads and writes bytes without text encoding changes.
...
The source is an existing regular file and the destination's parent exists.
Let unrelated I/O errors propagate; do not hide them as successful copies.

## Repair
The broken function uses a mode that destroys an existing destination.
Fix the open mode and handle only the expected collision exception.
