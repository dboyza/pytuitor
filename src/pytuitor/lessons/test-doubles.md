...
A function that controls timing or external I/O is easier to test when its dependencies are arguments.
You can pass a real operation in production and a small fake in tests.
A callable is any object you can invoke with parentheses, including functions and mocks.
...
Inside an exception handler, a bare `raise` re-raises the current exception with its traceback.
